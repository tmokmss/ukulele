 ...
 */
export function detectPitch(db, binHz) {
    ...
    const mag = (i) => (i < db.length ? Math.pow(10, Math.max(db[i], -140) / 20) : 0);
    ...
    for (let i = lo; i <= hi; i++) {
        ...
        if (!(db[i] > db[i - 1] && db[i] >= db[i + 1]))
            continue;
        let score = 0;
        for (let h = 0; h < HARMONIC_W.length; h++)
            score += HARMONIC_W[h] * magNear(i * (h + 1));
        if (score > bestScore) {
            bestScore = score;
            bestBin = i;
        }
...
export function nearestString(midi) {
    ...
    for (let i = 1; i < OPEN.length; i++) {
        if (Math.abs(midi - OPEN[i]) < Math.abs(midi - OPEN[best]))
            best = i;
    }
    const cents = (midi - OPEN[best]) * 100;
    if (Math.abs(cents) > MAX_STRING_CENTS)
        return null;
