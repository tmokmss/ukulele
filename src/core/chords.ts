...
export const NOTE = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'] as const;

/** 開放弦の MIDI ノート番号。G4 C4 E4 A4 (4弦 → 1弦)、High-G 前提 */
export const OPEN = [67, 60, 64, 69] as const;

export const STRING_NAME = ['4弦', '3弦', '2弦', '1弦'] as const;

/** フレット番号は 4弦→1弦 (G C E A) の順 */
export const CHORDS: Record<string, [number, number, number, number]> = {
  C: [0, 0, 0, 3],
  Am: [2, 0, 0, 0],
  F: [2, 0, 1, 0],
  G: [0, 2, 3, 2],
...
export type ChordTemplate = {
  ...
  /** 重複を除いた構成音のピッチクラス */
  pcs: number[];
  /** コサイン類似度の分母に使うテンプレート側のノルム */
  norm: number;
};

export const TEMPL: Record<string, ChordTemplate> = {};
...
export function weakLabel(chord: string, pc: number): string {
  const strs = TEMPL[chord].stringPcs
    .map((p, i) => (p === pc ? (STRING_NAME[i] as string) : null))
