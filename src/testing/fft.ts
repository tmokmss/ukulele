...
export function blackman(n: number, N: number): number {
 ...
 */
export function spectrumDb(signal: Float64Array): Float32Array {
  const N = signal.length;
  const re = new Float64Array(N);
  const im = new Float64Array(N);
  for (let i = 0; i < N; i++) re[i] = signal[i] * blackman(i, N);
  fft(re, im);
