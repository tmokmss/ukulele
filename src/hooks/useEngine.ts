...
import { TrainerEngine, type FrameInfo } from '../audio/engine';
import type { Settings } from '../core/types';

/** エンジンはアプリに1つ。初期設定だけ受け取り、以降の更新は setSettings で流す */
export function useEngine(settings: Settings): TrainerEngine {
 ...
 */
export function useFrame(engine: TrainerEngine, cb: (f: FrameInfo) => void): void {
  const ref = useRef(cb);
  useLayoutEffect(() => {
    ref.current = cb;
  });
