...
import { COUNT_IN } from '../audio/engine';
import { hitsInRange, placeRange } from '../core/timeline';
 ...
 */
export function ChordLane({ engine, tl, anchor, verdicts }) {
    ...
    const cards = useMemo(() => placeRange(tl, Math.max(0, anchor - PAST), Math.max(AHEAD - 1, anchor + AHEAD)), [tl, anchor]);
    ...
    return (_jsxs("div", { className: "chordlane", ref: laneRef, "aria-hidden": "true", children: [_jsx("i", { className: "gatezone", style: { left: gateX - CARD_W * 0.6, width: CARD_W * 1.2 } }), _jsx("i", { className: "gate", style: { left: gateX } }), _jsx("div", { className: "gate-count", ref: countRef, style: { left: gateX } }), _jsxs("div", { className: "strip", ref: stripRef, children: [Array.from({ length: Math.max(0, Math.round(to - from)) }, (_, i) => {
                        const beat = from + i;
                        const head = cards.some((c) => c.startBeat === beat);
                        return (_jsx("i", { className: head ? 'tick head' : 'tick', style: { left: beat * pxPerBeat } }, `t${beat}`));
                    }), hits.map((h) => (_jsx("i", { className: `hit h-${h.dir}`, style: { left: h.beat * pxPerBeat }, children: h.dir === 'D' ? '↓' : h.dir === 'U' ? '↑' : '×' }, `h${h.beat}`))), cards.map((c) => {
                        // 採点はゲートを通過したカードに直接出す。結果が付いたカードは薄くしない
                        const v = verdicts.get(c.index);
                        const state = c.index === anchor ? ' played' : c.index < anchor ? ' past' : c.index === anchor + 1 ? ' due' : '';
                        return (_jsxs("div", { ref: (el) => {
                                cardEls.current.set(c.index, el);
                                if (!el)
                                    cardEls.current.delete(c.index);
                            }, className: `ct${state}${v ? ` judged j-${v.tone}` : ''}`, style: { left: c.startBeat * pxPerBeat - CARD_W / 2, width: CARD_W }, children: [_jsx(FretMini, { chord: c.chord, w: 42 }), _jsx("span", { className: "nm", children: c.chord }), v && (_jsxs("span", { className: "vd", children: [v.mark && `${v.mark} `, v.label] }))] }, c.index));
                    })] })] }));
}
