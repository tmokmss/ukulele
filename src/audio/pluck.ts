 ...
 * お手本の音。
 *
 * Karplus-Strong (ノイズを弦の長さぶん遅らせながら平均していく) で弾いた弦の音を作り、
 * AudioBuffer に焼いて使い回す。音ごとに1回焼けば、あとは鳴らすだけなので、
 * 速い曲で毎拍4本ぶん鳴らしても間に合う。
 *
 * 鳴らす時刻は AudioContext.currentTime 基準で、engine.ts のクリックと同じ土俵に置く。
 */
import type { Stroke } from '../core/timeline';
 ...
 */
export function pluckSamples(sampleRate: number, midi: number, lenSec = LEN_SEC): Float32Array {
  const f = freqOf(midi);
  ...
  for (let h = 1; h <= harmonics; h++) {
    ...
    for (let i = 0; i <= n; i++) d[i] += amp * Math.sin((2 * Math.PI * h * i) / period + phase);
  }
  for (let i = 0; i <= n; i++) peak = Math.max(peak, Math.abs(d[i]));
  if (peak > 0) for (let i = 0; i <= n; i++) d[i] /= peak;

  for (let i = n + 1; i < len; i++) d[i] = (d[i - n] + d[i - n - 1]) * 0.5 * damp;
...
function render(ctx: BaseAudioContext, midi: number): AudioBuffer {
...
function renderMute(ctx: BaseAudioContext): AudioBuffer {
...
export class Pluck {
  ...
  private mute: AudioBuffer | null = null;
  ...
  private buf(midi: number): AudioBuffer {
  ...
  private fire(t: number, buf: AudioBuffer, gain: number): void {
   ...
   */
  strum(t: number, midi: number[], dir: Stroke): void {
    if (dir === 'x') {
      if (!this.mute) this.mute = renderMute(this.ctx);
      this.fire(t, this.mute, 1);
      return;
    }
    if (midi.length < 4) return;
    const down = dir === 'D';
